...
const formatLeaderNames = (leaderNames) => {
	...
	const names = leaderNames.split(",").map(name => name.trim());

	if (names.length === 1) return names[0];
	if (names.length === 2) return `${names[0]} and ${names[1]}`;

	const lastIndex = names.length - 1;
	const namesExceptLast = names.slice(0, lastIndex).join(", ");
	return `${namesExceptLast}, and ${names[lastIndex]}`;
};
...
export const ResponseModal = ({ id, entry, setAppStatus, setResponseModal, responseModal, refers }) => {
	...
	const [subject, setSubject] = useState(
		responseModal.type === "accept"
			...
			: `${EMAILS_SUBJECTS[responseModal.type]}: ${entry["Venue"]}`
	);
